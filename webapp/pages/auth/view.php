<?php
    ...
    if($_SESSION['s_userType'] == 'MANAGER'){
         ...
         $api_url = $BASE_URL_PRIMAVERA . 'clientes/'.$_SESSION['s_id'];
        $user = json_decode(file_get_contents($api_url), true);

        $api_url = $BASE_URL_PRIMAVERA . 'encomendas';
        $packagesAux = json_decode(file_get_contents($api_url), true);
        ...
        $api_url = $BASE_URL_PRIMAVERA . 'detalhesutilizadores/'.$_SESSION['s_id'];
        $user = json_decode(file_get_contents($api_url), true);
